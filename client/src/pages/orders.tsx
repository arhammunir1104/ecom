      ...
      if (uid) {
        ...
        try {
          ...
          if (!querySnapshot.empty) {
            console.log(
              `Found ${querySnapshot.docs.length} orders in top-level collection`,
            );
          ...
          if (!userOrdersSnapshot.empty) {
            console.log(
              `Found ${userOrdersSnapshot.docs.length} orders in user subcollection`,
            );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(apiOrders) && apiOrders.length > 0) {
            ...
            // Only add orders that aren't already in the array (by ID)
            const existingIds = allOrders.map((o) => o.id);
            const newApiOrders = formattedApiOrders.filter(
              (o) => !existingIds.includes(o.id),
            );
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {(order.items as any[]).reverse().map((item, index) => (
                          <div
                            key={index}
